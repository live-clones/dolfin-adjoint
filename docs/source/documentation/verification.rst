...
The main function in dolfin-adjoint for applying the Taylor remainder convergence test is :py:func:`taylor_test <dolfin_adjoint.taylor_test>`.
...
We set :py:data:`parameters["adjoint"]["stop_annotating"]` to :py:data:`True`, so that the :py:func:`solve <dolfin_adjoint.solve>` calls
in the perturbed runs of :py:func:`main` are not registered by libadjoint.
...
always be 1. (If they are not, try passing the :py:data:`seed` argument to :py:func:`taylor_test <dolfin_adjoint.taylor_test>` to
...
precisely where an error might lie; these are discussed in the :doc:`section on debugging
<debugging>`.
